...
export const EmployeeDetailsCard: React.FC<EmployeeDetailsCardProps> = ({ employee,selectedProjectId, onClose }) => {
    ...
    useEffect(() => {
        const fetchProjectHistory = async () => {
            ...
            try {
                const response = await getEmployeeProjectHistory(employee.id);

const allocations = response?.data || [];

const selectedProjectAllocations = allocations.filter((item: any) => {
  const projectId =
    item.projectId ||
    item.project_id ||
    item.project?.id;
...
const history = selectedProjectAllocations.map((item: any) => ({
  projectName:
    item.projectName ||
    item.project_name ||
    item.project?.name ||
...
setProjectHistory(history);
    ...
    const phone = personalInfo.contactNo || employee.contactNo || '';
    const joinedDate = personalInfo.joinDate || employee.joinDate || '';
            ...
            <div className="flex flex-col md:flex-row items-center md:items-center gap-6 md:gap-8 pb-2 border-b border-gray-100">
                ...
                <div className="flex-1 flex flex-col items-center md:items-start gap-1">
                    <div className="flex flex-col md:flex-row md:items-end gap-2">
                        ...
                        <span className="text-lg text-gray-500 font-medium">{employee.role || employee.designationName}</span>
                    </div>
                    <div className="flex flex-wrap gap-2 mt-1">
                        <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-semibold">{employee.isActive?'Active':''}</span>
                    </div>
                </div>
                {/* Availability Donut */}
